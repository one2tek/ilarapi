...
namespace one2tek\ilarapi\Controllers;

use JsonSerializable;
use Illuminate\Support\Str;
use Illuminate\Http\JsonResponse;
use Illuminate\Routing\Controller;
use one2tek\ilarapi\Core\Architect;
use Illuminate\Contracts\Support\Arrayable;
use one2tek\ilarapi\Exceptions\ilarapiException;
...
abstract class LaravelController extends Controller
{
    ...
    protected function parseFilters(array $filters, $or = false, $defaultOperator = 'eq')
    {
        ...
        foreach ($filters as $column => $part) {
            ...
            if ($arrayCountValues > 2) {
                throw new IlarapiException('Filter is not well formed.');
            }

            if (!in_array($operator, $allowedOperators)) {
                throw new IlarapiException('Operator '. $operator. ' is not supported.');
            }
    ...
    protected function parseFilterGroups(array $filter_groups)
    {
        ...
        foreach ($filter_groups as $group) {
            if (!array_key_exists('filters', $group)) {
                throw new IlarapiException('Filter group does not have the \'filters\' key.');
            }
            $filters = array_map(function ($filter) use ($keysNeeded) {
                if (count(array_intersect_key(array_flip($keysNeeded), $filter)) != count($keysNeeded)) {
                    throw new IlarapiException('You need to pass column, operator and value in filters.');
                }
                
                if (($filter['operator'] == 'in') && (!is_array($filter['value']))) {
                    throw new IlarapiException('You need to make value as array because you are using \'in\' operator.');
                }

                if (($filter['operator'] == 'bt') && (!is_array($filter['value']))) {
                    throw new IlarapiException('You need to make value as array because you are using \'bt\' operator.');
                }
    ...
    private function validateResourceOptions(array $data)
    {
        if ($data['page'] !== null && $data['limit'] === null) {
            throw new IlarapiException('Cannot use page option without limit option.');
        }
    }
}